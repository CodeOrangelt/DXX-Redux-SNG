...
gameseq_init_network_players()
{
	...
	if (Game_mode & GM_MULTI) {
		...
		for (; k < MAX_PLAYERS; k++) {
			...
			// coop_single_start's comment above). Cloning Objects[0]'s exact
			// position for every extra slot put all of them on top of each
			// other. Walk to a random connected neighboring segment per
			// extra slot instead -- same decorrelation trick survival.c's
			// own robot spawner uses -- so each extra player lands nearby
			// but not literally coincident.
