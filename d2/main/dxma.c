...
void dxma_missions_menu(void)
{
	...
	// Centre the visible text before wrapping it in the color escape.
	...
	m[1].text = ListText + DXMA_ROW_TEXT_LEN; m[1].type = NM_TYPE_TEXT;
	{
		...
		unsigned char col = FilterEnabled
			? (unsigned char)BM_XRGB(0, 31, 0)   // bright green when filter active
			: (unsigned char)BM_XRGB(31, 0, 0);  // bright red when filter inactive
		if (!col) col = 1;
		...
		snprintf(m[1].text, DXMA_ROW_TEXT_LEN, "\x01%c%s\x01\x99", col, filter_text);
	}
